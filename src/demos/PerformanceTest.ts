...
export class PerformanceTest extends BaseDemo {
  ...
  private _addArmature(): void {
    ...
    if (this._armatures.length === 0) {
      factory.parseDragonBonesData(
        Loader.shared.resources['dragon/mecha_1406/mecha_1406_ske.dbbin'].data
      )
    ...
    const armatureDisplay = dragonBones.PixiFactory.factory.buildArmatureDisplay(
      'mecha_1406'
    )
  ...
  private _resetPosition(): void {
    ...
    const dY =
      (this.stageHeight - paddingT - paddingB) /
      Math.ceil(armatureCount / columnCount)

    for (let i = 0, l = armatureCount; i < l; ++i) {
      const armatureDisplay = this._armatures[i]
      const lineY = Math.floor(i / columnCount)
      armatureDisplay.x =
        (i % columnCount) * dX + paddingHModify - this.stageWidth * 0.5
