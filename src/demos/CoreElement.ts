...
export class Game extends BaseDemo {
  ...
  public constructor() {
    ...
    factory.parseDragonBonesData(
      Loader.shared.resources['dragon/mecha_1502b/mecha_1502b_ske.json'].data
    )
    ...
    factory.parseDragonBonesData(
      Loader.shared.resources['dragon/skin_1502b/skin_1502b_ske.json'].data
    )
    factory.parseTextureAtlasData(
    ...
    factory.parseDragonBonesData(
      Loader.shared.resources['dragon/weapon_1000/weapon_1000_ske.json'].data
    )
    factory.parseTextureAtlasData(
...
class Mecha {
  ...
  private static readonly MAX_MOVE_SPEED_FRONT: number =
    Mecha.NORMALIZE_MOVE_SPEED * 1.4
  private static readonly MAX_MOVE_SPEED_BACK: number =
    Mecha.NORMALIZE_MOVE_SPEED * 1.0
  ...
  private static readonly WEAPON_L_LIST: Array<string> = [
    'weapon_1502b_l',
    'weapon_1005',
    'weapon_1005b',
    'weapon_1005c',
  ...
  private static readonly WEAPON_R_LIST: Array<string> = [
    'weapon_1502b_r',
    'weapon_1005',
    'weapon_1005b',
    'weapon_1005c',
  ...
  private static readonly SKINS: Array<string> = [
    'mecha_1502b',
    'skin_a',
    'skin_b',
    'skin_c',
  ...
  public constructor() {
    this._armatureDisplay = dragonBones.PixiFactory.factory.buildArmatureDisplay(
      'mecha_1502b'
    )
    this._armatureDisplay.x = 0.0
    this._armatureDisplay.y = Game.GROUND
    this._armature = this._armatureDisplay.armature
    this._armatureDisplay.on(
  ...
  public switchSkin(): void {
    ...
    const skinData = dragonBones.PixiFactory.factory.getArmatureData(skinName)
      .defaultSkin
    dragonBones.PixiFactory.factory.replaceSkin(
      this._armature,
      skinData,
      false,
      ['weapon_l', 'weapon_r']
  ...
  private _fire(firePoint: PointType): void {
    const radian =
      this._faceDir < 0 ? Math.PI - this._aimRadian : this._aimRadian
    const bullet = new Bullet(
      'bullet_01',
      'fire_effect_01',
      radian + Math.random() * 0.02 - 0.01,
      40,
      firePoint
    )
    ...
    if (this._moveDir === 0) {
      ...
      if (this._moveDir * this._faceDir > 0) {
        this._walkState.timeScale =
          Mecha.MAX_MOVE_SPEED_FRONT / Mecha.NORMALIZE_MOVE_SPEED
      } else {
        this._walkState.timeScale =
          -Mecha.MAX_MOVE_SPEED_BACK / Mecha.NORMALIZE_MOVE_SPEED
      }
  ...
  private _updateAim(): void {
    ...
    if (this._armatureDisplay.armature.flipX !== this._faceDir < 0) {
      this._armatureDisplay.armature.flipX = !this._armatureDisplay.armature
        .flipX
    ...
    const aimOffsetY =
      this._armature.getBone('chest').global.y * this._armatureDisplay.scale.y
    if (this._faceDir > 0) {
      this._aimRadian = Math.atan2(
        this._target.y - this._armatureDisplay.y - aimOffsetY,
...
class Bullet {
  ...
  public constructor(
    armatureName: string,
    effectArmatureName: string | null,
    radian: number,
    speed: number,
    position: PointType
  ) {
    ...
    if (effectArmatureName !== null) {
      this._effecDisplay = dragonBones.PixiFactory.factory.buildArmatureDisplay(
        effectArmatureName
      )
