...
export class AnimationBase extends BaseDemo {
  ...
  public constructor() {
    ...
    factory.parseDragonBonesData(
      Loader.shared.resources['dragon/progress_bar/progress_bar_ske.json'].data
    )
    factory.parseTextureAtlasData(
      Loader.shared.resources['dragon/progress_bar/progress_bar_tex.json'].data,
      Loader.shared.resources['dragon/progress_bar/progress_bar_tex.png']
        .texture
    )
    //
  ...
  private _touchHandler(event: interaction.InteractionEvent): void {
    const progress = Math.min(
      Math.max((event.data.global.x - this.x + 300.0) / 600.0, 0.0),
      1.0
    )
    switch (event.type) {
      case 'touchstart':
      ...
      case 'mousemove':
        if (this._isTouched) {
          const animationState = this._armatureDisplay.animation.getState(
            'idle'
          )
          if (animationState) {
