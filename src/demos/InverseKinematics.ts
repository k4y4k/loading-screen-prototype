...
export class InverseKinematics extends BaseDemo {
  ...
  public constructor() {
    ...
    factory.parseDragonBonesData(
      Loader.shared.resources['dragon/mecha_1406/mecha_1406_ske.json'].data
    )
    ...
    factory.parseDragonBonesData(
      Loader.shared.resources['dragon/floor_board/floor_board_ske.json'].data
    )
    factory.parseTextureAtlasData(
  ...
  private _updateAim(): void {
    ...
    if (this._faceDir > 0) {
      this._aimRadian = Math.atan2(
        this._target.y - positionY - aimOffsetY,
        this._target.x - positionX
      )
    } else {
      this._aimRadian =
        Math.PI -
  ...
  private _updateFoot(): void {
    ...
    let circleRadian = Math.atan2(
      this._circleBone.global.y,
      this._circleBone.global.x
    )

    if (this._circleBone.global.x < 0.0) {
      circleRadian = dragonBones.Transform.normalizeRadian(
        circleRadian + Math.PI
      )
    }

    this._offsetRotation = Math.min(
      Math.max(circleRadian, minRadian),
      maxRadian
    )
    ...
    this._leftFootBone.offset.y =
      tan * this._leftFootBone.global.x + this._leftFootBone.origin.y * sinR
    ...
    this._rightFootBone.offset.y =
      tan * this._rightFootBone.global.x + this._rightFootBone.origin.y * sinR
  ...
  destroy(options?: {
    ...
    DragHelper.getInstance().disableDrag(
      this._floorBoard.armature.getSlot('circle').display
    ) //technically unneeded, but to be extra safe
