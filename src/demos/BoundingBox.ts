...
export class BoundingBox extends BaseDemo {
  ...
  public constructor() {
    ...
    factory.parseDragonBonesData(
      Loader.shared.resources['dragon/mecha_2903/mecha_2903_ske.json'].data
    )
  ...
  private _enterFrameHandler(_deltaTime: number): void {
    ...
    this._armatureDisplay.toLocal(
      this._helpPointA,
      this._boundingBoxTester,
      this._helpPointA
    )
    this._armatureDisplay.toLocal(
      this._helpPointB,
    ...
    const containsSlotA = this._armatureDisplay.armature.containsPoint(
      this._helpPointA.x,
      this._helpPointA.y
    )
    const containsSlotB = this._armatureDisplay.armature.containsPoint(
      this._helpPointB.x,
      this._helpPointB.y
    )
...

    {
      ...
      if (
        this._boundingBoxTester.animation.lastAnimationName !== animationName
      ) {
        this._boundingBoxTester.animation.fadeIn(
          animationName,
          0.2
        ).resetToPose = false
      }
      ...
      if (intersectsSlots) {
        this._boundingBoxTester.toLocal(
          this._helpPointA,
