...
export class ReplaceSkin extends BaseDemo {
  ...
  private readonly _factory: dragonBones.PixiFactory =
  ...
  public constructor() {
    ...
    this._factory.parseDragonBonesData(
      Loader.shared.resources['dragon/you_xin/body/body_ske.json'].data
    )
    this._factory.parseTextureAtlasData(
      Loader.shared.resources['dragon/you_xin/body/body_tex.json'].data,
      Loader.shared.resources['dragon/you_xin/body/body_tex.png'].texture
    )
    ...
    for (let i = 0, l = this._suitConfigs.length; i < l; ++i) {
      for (const partArmatureName of this._suitConfigs[i]) {
        ...
        //
        this._factory.parseDragonBonesData(
          Loader.shared.resources[dragonBonesJSONPath].data
        )
        this._factory.parseTextureAtlasData(
          Loader.shared.resources[textureAtlasJSONPath].data,
          Loader.shared.resources[textureAtlasPath].texture
        )
      }
    }
    //
  ...
  private _animationEventHandler(_event: dragonBones.EventObject): void {
    ...
    const animationName = this._armatureDisplay.animation.animationNames[
      animationIndex
    ]
    // Play animation.
  ...
  private _randomReplaceSkin(): void {
    ...
    if (this._replaceSuitParts.length === 0) {
      ...
      for (const partArmatureName of this._suitConfigs[
        this._replaceSuitIndex
      ]) {
